...
namespace Portals
{
    ...
    internal partial class VisibilityTree
    {
        ...
        private List<VisibilityTree.Node> dependencies = new List<VisibilityTree.Node>();
        ...
        internal VisibilityTree(PortalRenderer renderer)
        {
            ...
            foreach (Portal p in PortalManager.Instance.Portals)
                if (p.IsVisibleFrom(this.renderer.BaseCamera))
                    this.dependencies.Add(new VisibilityTree.Node(this.renderer, p, renderer.BaseCamera.transform.localToWorldMatrix));
        }
